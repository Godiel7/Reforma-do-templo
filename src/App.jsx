        ...
        <div key={idx} className="mb-6 p-4 bg-gray-50 rounded-lg">
          ...
          {section.questions.map((q, i) => (
            <div key={i} className="flex items-center gap-3 mb-2">
              <input
                type="checkbox"
                checked={decisionData[Object.keys(decisionData)[idx + 1]][i] || false}
                onChange={e => {
                  const key = Object.keys(decisionData)[idx + 1];
                  const arr = [...decisionData[key]];
                  arr[i] = e.target.checked;
                  setDecisionData({...decisionData, [key]: arr});
                }}
              />
...
// === OUTROS COMPONENTES ===
const MetricasTab = ({ title, metrics, setMetrics }) => { /* ... mesmo de antes ... */ };
const ListaHistorico = ({ data, setSelectedDate, setActiveTab }) => { /* ... mesmo de antes ... */ };
